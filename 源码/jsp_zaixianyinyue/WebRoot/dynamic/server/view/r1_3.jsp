...
<script type="text/javascript">
	...
	function deleteUser(){
		...
		if(selectedRows.length==0){
			$.messager.alert("系统提示","请选择要选择的删除的会员！");
			return;
		}
		var strIds=[];
		...
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确认要删除这<font color=red>"+selectedRows.length+"</font>个会员吗？",function(r){
			if(r){
				$.post("<%=basePath%>/server/ActionR1_deleteUser.action",{delIds:ids},function(result){
					if(result.success){
	...
	function openUserAddDialog(){
		...
		url="<%=basePath%>/server/ActionR1_addShiyan.action";
	}
	
	function openUserModifyDialog(){ 
		var selectedRows=$("#dg").datagrid('getSelections');
		if(selectedRows.length!=1){
			$.messager.alert("系统提示","请选择一条要评分的数据！");
			return;
		}
		...
		$("#title").val(row.title);
		$("#msg").val(row.msg);
		url="<%=basePath%>/server/ActionR1_updateScore.action?id="+row.id;
	}
...
<body style="margin: 5px;">
	<table id="dg" title="我的信息" class="easyui-datagrid" fitColumns="true"
	 pagination="true" rownumbers="true" url="<%=basePath%>/server/ActionR1_selectUser.action" fit="true" toolbar="#tb">
		<thead>
			<tr>
					<th field="cb" checkbox="true"></th>
					<th field="id" width="10" align="center">编号</th>
					<th field="account" width="15" align="center">用户账户</th>
					<th field="pwd" width="15" align="center">用户密码</th>
					<th field="name" width="15" align="center">用户名</th>
					<th field="sex" width="15" align="center">用户性别</th>
					<th field="email" width="15" align="center">用户邮箱</th>
			</tr>
		</thead>
	</table>
	...
	<div id="tb">
		<div>
			...
			<a href="javascript:openUserModifyDialog2()" class="easyui-linkbutton" iconCls="icon-remove" plain="true">修改用户</a>
			<a class="easyui-linkbutton" iconCls="icon-remove" plain="true">用户查询：</a>
			<input type="text" value="" placeholder="请输入用户名" style="height: 10px;width: 100px"/>
			     <input type="button" value="查询" name="chaxun" style="width: 40px;height: 20px"/> 
		</div>
	</div>
	...
	<div id="dlg" class="easyui-dialog" style="width: 400px;height: 300px;padding: 10px 20px"
		...
		<form id="fm" method="post">
			<table>
				...
				<tr>
					<td>实验内容</td>
					<td><textarea rows="11" cols="1" name="msg" id="msg" required="true" ></textarea></td>
				</tr>
				<tr>
					<td>评分</td>
					<td><input type="text" name="score" id="score" class="easyui-validatebox" required="true"/></td>
				</tr>
			</table>
		</form>
	</div>
